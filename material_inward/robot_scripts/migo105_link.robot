*** Settings ***
Documentation       SAP MIGO 105 - Attach Invoice Document URL via GOS Toolbox (MIGO 105 link bot)
...                 Same recorded-VBS flow as migo103_link.robot (Login -> MIGO
...                 -> enter material doc -> GOS Toolbox -> Create URL -> link
...                 -> confirm), targeting the MIGO 105 material document
...                 instead. Requires: pip install pywin32
...
...                 Called by services/rf_runner.py's execute_migo105_link_sap()
...                 as its own separate RF-queue job, after MIGO 105 posting has
...                 already succeeded -- a failure here never affects whether
...                 MIGO 105 itself succeeded. MATERIAL_DOC_NUMBER / DOCUMENT_LINK
...                 are passed in via --variable; RESULT:MIGO105_LINK_STATUS:SUCCESS
...                 at the end of test case 04 is how rf_runner.py confirms the
...                 link was actually attached, on top of Robot Framework's own
...                 exit code.
...
Initialize Logging And Environment
    ...
    Append To File    ${LOG_FILE}    BOT EXECUTION LOG - MIGO105 INVOICE LINK\n
    Append To File    ${LOG_FILE}    ═══════════════════════════════════════════════════════════════\n
...
04 Create Invoice URL Via GOS Toolbox
    ...
    TRY
        ...
        # execute_migo105_link_sap() via _extract_marked_value() -- confirms
        ...
        Log To Console    RESULT:MIGO105_LINK_STATUS:SUCCESS
    EXCEPT    AS    ${error}
