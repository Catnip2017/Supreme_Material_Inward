...
Initialize SAP And Login
    ...
    # v16: per-user SAP credential pass-through (LDAP users only). If
    # rf_runner.py attached this job's own SAP_USER_OVERRIDE/
    # SAP_PASS_OVERRIDE to the subprocess environment, use those instead
    # of the shared spl_rpa .env account. Local/test-account jobs never
    # have these set and fall straight through to .env, unchanged from
    # before. ${PASSWORD} is never logged in either branch.
    ...
    Should Not Be Empty    ${USERNAME}       SAP username not resolved (no override, and SAP_USERNAME not found in .env)
    Should Not Be Empty    ${PASSWORD}       SAP password not resolved (no override, and SAP_PASSWORD not found in .env)
